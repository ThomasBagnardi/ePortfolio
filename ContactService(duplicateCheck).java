...
public class ContactService {
	// Start with an ArrayList of contacts to hold the list of contacts
		...
		// Checks for duplicates using the contactList as a parameter.
		// Returns true if duplicates are found, otherwise returns false.
		public static boolean duplicateCheck(ArrayList<Contact> contactList) {
			Set<Contact> seenContacts = new HashSet<>();
			// add() method returns false if element already exists.
			if (!seenContacts.add(contact)) {
				return true; // Duplicate has been found
			}
		}
		return false; // No duplicates have been found
}